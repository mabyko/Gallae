...
| 진입점 | Remotes → 대상 Remote의 Test Connection |
| 완료 상태 | 성공하면 `Reachable`이 표시되고 Repository의 local 상태는 유지된다. |

## 정상 흐름

1. 사용자가 Remotes를 열고 대상 Remote의 Test Connection을 누른다.
...
- Push URL과 쓰기 권한은 시험하지 않는다. Remote 이름 변경은 같은 Remotes 목록의 Edit에서 제공한다.
