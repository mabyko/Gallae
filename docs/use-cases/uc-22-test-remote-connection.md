...
| 진입점 | Navigator 범위 목록에서 remote 선택 → History 헤더의 `Edit…` → 시트 아래의 `Test Connection` |
| 완료 상태 | 성공하면 `Reachable`이 표시되고 Repository의 local 상태는 유지된다. |

## 정상 흐름

1. 사용자가 Navigator에서 remote를 선택하고 History 헤더의 `Edit…`로 연 시트에서 `Test Connection`을 누른다.
...
- Push URL과 쓰기 권한은 시험하지 않는다. 시험은 저장된 Fetch URL을 쓰므로 URL을 고쳤다면 먼저 Save 한다.
