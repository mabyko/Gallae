...
- 연결된 Worktree가 있으면 Branches 위에 접을 수 있는 Worktrees 목록을 표시한다. 기본 폴더와 연결된 폴더를 나란히 나열하고 branch 또는 Detached HEAD, 현재 폴더, 잠금·누락 상태를 구분한다. 한 번 선택하면 History에서 해당 HEAD를 확인하고, 더블클릭·Return·Open Worktree는 폴더를 연다. CLI나 다른 앱에서 만든 Worktree도 같은 목록에 표시한다.
