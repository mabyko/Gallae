...
- 선택 전환과 새로고침은 이전에 읽은 내용을 유지한 채 제자리에서 갱신하고, 로드가 길어질 때만 진행 표시로 바꾼다. 같은 Repository를 새로 읽을 때 History·Stashes의 선택한 revision과 파일을 유지한다. 선택 대상이 사라지면 남은 항목으로 이동하고, 다른 revision이나 Repository를 고르면 이전 파일 선택을 초기화한다.
...
- 작은 창에서는 보조 영역부터 접고, 선택 항목과 핵심 동작은 남긴다. History·Stashes의 저장된 변경 칸이 560pt보다 좁으면 파일 목록을 경로 선택 메뉴로 바꿔 diff 폭을 확보한다. 넓어지면 같은 선택으로 목록을 복원한다.
- History의 긴 커밋 본문은 네 줄 미리보기와 Show Full Message로 제공한다. 펼친 본문은 별도 스크롤로 끝까지 읽고 Show Less로 접으며, 다른 커밋을 고르면 다시 접힌다.
- 콘텐츠 패널 분할선은 드래그 외에 키보드 포커스 후 좌우 화살표와 VoiceOver 증감 동작으로 조절할 수 있다.
...
- **작업공간 구조.** Navigator · 목록 · 내용의 3열. Navigator는 목록 둘이다(시안 7, A1). 위는 화면 목록(Workspace: Changes·History, Recovery: Stashes·Reflog)으로 항상 하나가 선택돼 있고, 아래는 범위 목록(Branches·Remotes와 그 아래 remote branch·Tags)으로 선택이 있어도 없어도 된다. 범위를 고르면 History가 그 범위로 바뀌고, 화면 목록에서 화면을 고르면 범위 선택이 풀린다(두 선택이 서로 다른 것을 가리키지 않게). 목록이 둘이라 History 안에서는 두 선택이 동시에 보이고, 포커스 없는 쪽은 macOS가 회색으로 그린다. branch 한 번 클릭은 보기, 두 번 클릭은 전환이며 전환해도 화면은 그대로다. HEAD는 굵은 글씨와 표시로 선택과 따로 보인다. remote는 화면이 아니라 범위라 별도 Remote 화면이 없다. Fetch·Fetch & Prune·Edit…는 History 헤더의 도구이고, URL·Test Connection·Remove는 Edit… 시트 안이며(Remove는 시트 오른쪽 위), remote 행의 우클릭 메뉴에도 같은 동사가 있다. Stashes는 개수만 표시하고 항목은 본문에 둔다. 헤더는 한 줄 문맥 바(branch 메뉴 · Tracking · ahead/behind · working tree · 마지막 Fetch)다.
...
- **줄 단위 staging.** diff의 추가·삭제 줄에 거터 체크박스가 있고(Unified·Split 모두) hunk 헤더의 체크박스가 hunk 전체를 고른다. hunk 버튼은 고른 줄이 없으면 hunk 전체, 있으면 고른 줄만 stage·unstage 한다. Working Tree 섹션에는 확인을 거치는 Discard 버튼이 하나 더 있고, untracked 파일도 고른 줄만으로 index에 올릴 수 있다(구현 계획 7C). 둘을 설정으로 고르게 하지 않는다.
...
한 파일이 staged 이면서 동시에 수정돼 있을 때 "무엇이 커밋에 들어가고 무엇이 안 들어가는가"를 읽는 방법을 정한 기록이다. 시안 8은 1180pt 창의 일회용 HTML이며 시안 3~7처럼 로컬에만 둔다.

계기는 diff 칸에 Staged와 Working Tree가 같은 무게의 배지로 붙어 한 덩어리로 읽히던 것이다. 현재 화면에서 문제 일곱을 확인했다. 섹션이 하나뿐일 때도 배지가 붙고, 둘일 때 위계가 없고, 새 파일인데 Split이 왼쪽 절반을 잡고, `+`·`-` 접두사가 중복이면서 문맥 줄과 어긋나 보이고, `@@ …`와 `new file mode`가 raw git 문법이고, Staged hunk에는 동작이 없고, 짧은 파일 아래로 빈 공간이 압도한다.

검토한 안은 넷이다. A는 Staged를 접는다. B는 둘 다 펼쳐 두고 index 쪽에 초록 왼쪽 띠를 준다. C1은 세그먼트로 한 번에 하나만 보여 준다. C2는 C1에 반대편 한 줄 요약을 더한다.

**C2로 정했다.** 판단의 축은 "Stage Hunk를 눌렀을 때 그것이 어디로 갔는지 알 수 있는가"였다. 화면 밖에 있는 목적지는 화면 안에 대리물이 있어야 한다. C2의 반대편 요약 줄이 그 대리물이고, stage 하는 순간 그 줄의 숫자가 제자리에서 늘어나며 잠깐 빛나 "사라졌다"가 아니라 "저기로 갔다"가 된다. C1에는 그 대리물이 없어 작은 숫자 하나에 의존한다. B는 이동이 직접 보이지만 세로를 항상 두 배 쓴다. C2의 세로 비용은 머리 두 줄로 **일정**한 반면 B는 내용이 길수록 늘어난다.

A는 펼치면 B와 같은 화면이 되어 차이가 기본값 하나뿐이었고, 유일한 섹션은 접을 수 없어 "둘 다 있을 때만 접는다"는 조건부 기본값이 필요했다. 그런데 둘 다 있을 때가 바로 둘 다 보고 싶은 때라 제외했다.

함께 정한 것들.

- 섹션이 하나뿐인 파일에는 구획 머리를 두지 않는다. 대부분의 파일이 그렇고 그때 그 배지는 정보가 없다.
- hunk 머리는 위치를 사람 말로 쓰고 raw `@@ …`는 흐리게 옆에 남긴다. Git을 아는 사람은 계속 읽을 수 있다.
- `+`·`-` 접두사를 없애고 왼쪽 색 막대로 옮긴다. 문맥 줄과 코드가 같은 열에서 시작한다.
- 새 파일과 삭제된 파일은 비교할 반대편이 없으므로 Split에서도 한 칸으로 그린다.
- 마지막 줄을 stage 해 한쪽이 비면 빈 칸 대신 무슨 일이 일어났는지와 다음 행동을 둔다. **자동으로 탭을 바꾸지 않는다.** 사용자가 누르지 않은 전환은 놀라움이 된다.
- Staged hunk에도 Unstage Hunk와 거터 체크박스를 둔다. 지금 없는 것은 새로 추가된 파일의 부분 unstage가 막혀 있기 때문이고, 이는 별도로 고친다.
